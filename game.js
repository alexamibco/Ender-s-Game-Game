window.onload = function () {
  alert(
    "Ayuda a la nave de Ender a llegar al planeta. Usa las teclas de flecha para mover la nave y esquivar los asteroides."
  );
};
...
function moveSpaceship(dx, dy) {
  spaceshipX = Math.min(
    Math.max(0, spaceshipX + dx),
    gameContainer.clientWidth - spaceship.offsetWidth
  );
  spaceshipY = Math.min(
    Math.max(0, spaceshipY + dy),
    gameContainer.clientHeight - spaceship.offsetHeight
  );
